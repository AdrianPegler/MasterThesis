...
$G$ durch Matrizen $A$ und $B$ durch $G = A\trans{B}$.
	...
	\mathfrak{I} \colon C[-1,1] \to \Pi_k, \ \ \ \ \ \ \ \ \ \ \ \ f \mapsto \sum_{\nu \in \nullhaken{k}} f\left(\xi_\nu\right) \lag_\nu
      ...
      Somit können wir diese Funktionen bereits mit Hilfe der Stützstellen durch die Lagrange-Polynome annähern.
	...
	\Phi_{[a,b]} \colon [-1,1] \to [a,b] \ \ \ \ \ \ \ \ \ \ \ \  x \mapsto \frac{b + a}{2} + \frac{b - a}{2} x.
      \end{equation*}
      Diese Transformation hat für alle $x \in [-1,1]$ die Eigenschaften:
      \begin{equation*}
	...
	\mathfrak{I}_{[a,b]}[f] = \sum_{\nu \in \nullhaken{k}} f \after \Phi_{[a,b]}\left(\xi_\nu\right) \lag_\nu \after \Phi_{[a,b]}^{-1} = 
      ...
      \end{equation*}
      Da diese Definition recht unhandlich ist, definieren wir zur Vereinfachung für $\nu \in \nullhaken{k}$ und $x \in [a,b]$:
      \begin{align*}
	\xi_{[a,b],  \nu} &:= \Phi_{[a,b]}\left(\xi_\nu\right) = \frac{b + a}{2} + \frac{b - a}{2} \xi_\nu\\
	\lag_{[a,b], \nu} &:= \prod_{\substack{\mu \in \nullhaken{k} \\ \mu \neq \nu}} \frac{x - \xi_{[a,b],  \mu}}{\xi_{[a,b],  \nu} - \xi_{[a,b],  \mu}}.
      \end{align*}
      Da mit \autoref{eq:lag_delta} die Gleichung
      \begin{align*}
	...
	\mathfrak{I}_{Q,\iota} \colon C\left(Q\right) \to C\left(Q\right).
      \end{equation*}
      Mit diesem erhalten wir für $f \in C\left(Q\right)$ und $x \in Q$:
      \begin{equation*}
	...
	\mathfrak{I}_Q[f] = \sum_{\nu \in M} f\left(\xi_{Q,\nu}\right)\lag_{Q,\nu} \text{, für alle } f \in C\left(Q\right).
      ...
      Mit diesem Interpolationsoperator können wir nun die entartete Approximation der Kernfunktion $g$ aus \autoref{eq:gsigma} konstruieren. Der Einfachheit halber identifizieren wir im Folgenden
	...
	\textit{$\eta$-Zulässigkeitsbedingung}. Paare $\left(\tau, \sigma\right)$, für die $\mathcal{Z}\left(\tau, \sigma\right) = \ttit{zulässig}$ gilt, werden \textit{$\eta$-zulässig} genannt.
      ...
      \citet{nichtlokop} weist in Kapitel 3.4 nach, dass für asymptotisch glatte Funktionen die Approximation für jedes $\eta > 0$ exponentiell konvergiert, wenn die obige Zulässigkeitsbedingung erfüllt
	...
	\begin{enumerate}
	  ...
	  \item Zu jedem Knoten $v \in V \backslash \{root\left(T\right)\}$ gibt es $n \in \N$ und einen Weg $w := \left(v_i\right)_{i \in \nullhaken{n}}$ der Länge $n$ von der Wurzel $root\left(t\right) =: v_0$ zum 
	...
	\citep{nichtlokop}
      ...
      \item Die Partitionen sind über die Ebenen des Baumes hierarchisch geordnet. Damit ist gemeint, dass für alle $i \in \haken{depth\left(T_M\right)}$ die Partition $P^{\left(i\right)}$ feiner ist
	...
	Wir nennen einen Clusterbaum $T_{X \times Y}$ der Menge $X \times Y$ Blockbaum, falls die folgenden Eigenschaften erfüllt sind:
	\begin{enumerate}
	  ...
	  \end{equation*}
	\end{enumerate}
	Die Knoten des Blockbaumes heißen \textit{Blöcke}. Die Komponenten $t \in T_X$ und $s \in T_Y$ werden auch \textit{Zeilen-} und \textit{Spaltencluster}
	genannt.
...
%     \clearpage
    ...
    Im Folgenden werden wir uns einige Eigenschaften der \hmat genauer anschauen. Dadurch werden wir Möglichkeiten finden, durch die wir hierarchische Matrizen noch sparsamer speichern und 
    ...
    Die Spalten- (bzw. Zeilen-)matrizen lassen sich also verlustfrei aus den Spalten- (bzw. Zeilen-)matrizen der Söhne konstruieren.
    ...
    \end{equation*}
    Die Funktion $\tilde g$ ist ebenfalls eine entartete Kernfunktion, nur dass diesmal die Körper $x$ und $y$ ausschließlich in Verbindung mit den Lagrange-Polynomen auftauchen. Mit 
    \begin{equation*}
    ...
    Da die Matrizen $V_\tau$ analog zu den Matrizen $W_\sigma$ definiert wurden, teilen sie dieselben positiven Eigenschaften. Beide lassen sich also nicht nur effizient speichern, sondern auch 
