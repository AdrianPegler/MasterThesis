...
void _setup(Cluster *c, int depth){
  ...
  if(c->activ == semi_activ){
    ...
    Zudem bekommt der Konstruktor \code{_new_bound_Cluster} einen neuen Parameter um den Member \code{activ} aller Cluster zu initialisieren. Für die Konstruktion der Wurzel ist dieser auf
    \code{semi_activ} gesetzt. Danach wird durch die Methode \code{_setup_nonLeafCluster(...)} immer der Wert des Vaters an die Söhne weitergegeben.
    Der angepasste Code der \code{_setup}-Methode ist in \autoref{lst:parsetup} aufgeführt.
    
    Zunächst wird überprüft, ob die \code{SPLIT_DEPTH} erreicht wurde. Ist dies der Fall wird der Member \code{activ} auf 
    ...
    auf \code{semi-activ} bleiben, oder, falls kein Nachfahre aktiv ist, der Member \code{activ} auf \code{inactiv} gesetzt wird.
