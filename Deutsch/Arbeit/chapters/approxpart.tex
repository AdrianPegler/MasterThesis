...
typedef struct _Cluster Cluster;
    ...
    \label{sec:vorw}
    \input{chapters/forw}
    \todo{korrektur}Nun da wir den Clusterbaum $T_\Omega$ konstruiert haben, können wir uns an die Berechnung der Gravitation machen.
    ...
    \todo{korrektur}Da $W_\sigma$ ausschließlich von Clustern und nicht von Blöcken abhängt, können wir die Vorwärtstransformation einfach für alle $\sigma \in T_\Omega$ durchführen. Dies geschieht
    ...
    \todo{korrektur}Die Methode \code{\_forward\_Leaf(...)} entspricht der Konstruktion und Auswertung der Matrizen $W_\sigma$ anhand der Sonnen $x \in \sigma$. Durch sie werden, wie auf 
    ...
    \todo{korrektur}Die Methode \code{\_forward\_nonLeaf(Cluster *c)} nutzt die Struktur der geschachtelten Clusterbasis um die Matrizen $W_\sigma$ anhand der Sohncluster zu konstruieren.Sie berechnet die Ersatzmassen
    ...
    \todo{korrektur}Auf diese Weise durchlaufen die Methoden gemeinsam betrachtet rekursiv den Clusterbaum. Der Code der drei Methoden ist in \autoref{lst:forw}, \autoref{lst:forwnonleaf} und \autoref{lst:forwleaf}
